...
func (h *StudyHandler) UpdateStudy(c echo.Context) error {
	...
	stmt, err := h.DB.Prepare("select s.title, s.orientation, s.description from study s join user u on s.user_id == u.id where s.id == ?")
	if err != nil {
		log.Println(err)
	}
	defer stmt.Close()
	...
	var orientation string
	var description string
	stmt.QueryRow(studyID).Scan(&title, &orientation, &description)

	b["study_title"] = title
	b["study_id"] = studyID
	b["study_orientation"] = orientation
	b["study_description"] = description
	...
	b["study_pgn"] = string(content)

	return c.Render(http.StatusOK, "create_study.html", b)
}
...
func (h *StudyHandler) UpdateStudyPOST(c echo.Context) error {
	title := database.EscapeStringWithSpaces(c.FormValue("title"))
	description := database.EscapeStringDescription(c.FormValue("description"))
	if len([]rune(title)) <= 0 {
		return errors.New("Title cannot be empty")
	...
	id := studyIDFromURL(c.Request().URL.String())
	...
	stmt, err = tx.Prepare("insert or replace into study(id, user_id, title, orientation, description) values(?, ?, ?, ?, ?)")
	if err != nil {
		log.Println(err)
	}
	defer stmt.Close()
